...
void stream_task(void *pvParameters) {
    ...
    for (;;) {
        uint16_t task_rate = CONTROLS_FREQ; // Run every tick unless streaming rate dictates otherwise

        if (gimbal->state.streaming) {
            // Only pulse when streaming
            gpio_put(TEST_PIN, 1);

            // Adjust task rate based on stream rate
            task_rate = gimbal->state.stream_rate;
        ...
        vTaskDelayUntil(&last, pdMS_TO_TICKS(SECS2MSECS(FREQ2PERIOD(task_rate))));
    }
}
